...
libname perm0'D:\MainData\data';
libname perm'D:\DealerNetworkProject\Data';


libname check'D:\DealerNetworkProject\check';
...
%let startday='01JAN10'D;
%let RatingStartDay='1JAN09'D;
...
data data1;
set perm.RAWtotal(keep=RPTG_PARTY_ID CNTRA_PARTY_ID ENTRD_VOL_QT TRD_EXCTN_DT TRD_EXCTN_TM);
run;


data data1;
set data1;
Date=dhms(TRD_EXCTN_DT, 0,0,TRD_EXCTN_TM);
format Date dtpic.;
run;

data data1;
set data1(keep=RPTG_PARTY_ID CNTRA_PARTY_ID ENTRD_VOL_QT TRD_EXCTN_DT TRD_EXCTN_TM Date);
run;

proc export data=data1
    outfile="D:\DealerNetworkProject\code\Codes_CFH\Codes\Codes_Bond_Market\Codes_Bond_Market\data\data1.csv"
    dbms=csv
    replace;
    delimiter=",";
run;
